  ...
  return (
    <section className="journey-section" id="journey">
      <div className="section-container">
        ...
        {/* Chronological Roadmap Strip */}
        <div className="roadmap-horizontal-strip">
          <div className="roadmap-strip-header">
            <div className="strip-title-group">
              <span className="strip-dot" />
              <h4>CHRONOLOGICAL EVENT PIPELINE</h4>
            </div>
            <div className="opening-ceremony-tag">
              <Sparkles size={14} className="text-cyan" />
              <span>Opening Ceremony: <strong>24 August 2026</strong></span>
            </div>
          </div>

          <div className="roadmap-items-scroller">
            {events.map((evt, idx) => (
              <div
                key={evt.id}
                className={`roadmap-step-card ${idx === activeStageIndex ? "step-highlight" : ""}`}
                onClick={() => handleStageSelect(idx)}
              >
                <div className="step-num-top">{String(idx + 1).padStart(2, "0")}</div>
                <h5 className="step-card-name">{evt.name}</h5>
                <p className="step-card-summary">{evt.description}</p>
                <div className="step-card-footer">
                  <span className="step-card-date">{evt.date}</span>
                </div>
              </div>
            ))}
          </div>
